  ...
  const onSubmit = async (data) => {
   ...
   const isFieldFilled = (field) => {
     ...
     return typeof field === "object"
       ? field !== null && field !== undefined && Object.keys(field).length > 0
       : typeof field !== "object" &&
    ...
    console.log(data.duty_free.value)
     try {
      ...
      //console.log(dealData)
       // if (selectedFile) {
       //   dealData.image = selectedFile;
       // } else if (redux.dealData[id]?.image) {
       //   dealData.image = redux.dealData[id]?.image;
       // } else {
       //   dealData.image = '';
       // }
       ...
       if (error.response && error.response.status === 422) {
         ...
         if (errors.user_name) {
           ...
           setError("arrival_country", {
             type: "manual",
             message: errors.arrival_country
       ...
       if (!isFieldFilled(data[key])) {
         const errorMessage =
           ...
           (typeof data[key] === "object" &&
             ...
             ? `The ${key} field has no available options.`
